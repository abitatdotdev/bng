...
test("enrichWithTotalWatercourseUnitsSRM calculation with spatial risk", () => {
    const enriched = enrichWithWatercourseData({
        length: 1.2,
        lengthRetained: 0.8,
        lengthEnhanced: 0.2,
    ...
    // Add spatial risk multiplier
    const withSpatialRisk = {
        ...withUnits,
        spatialRiskMultiplier: 0.75, // "Compensation outside LPA or NCA of impact site, but in neighbouring LPA or NCA"
    };

    const result = enrichWithTotalWatercourseUnitsSRM(withSpatialRisk);

    // Expected: 1.2 * 6 * 2 * 1.1 * 0.85 * 0.67 * 0.75
    expect(result.totalWatercourseUnitsSRM).toBeCloseTo(1.2 * 6 * 2 * 1.1 * 0.85 * 0.67 * 0.75, 5);
});

test("enrichWithTotalWatercourseUnits calculation without spatial risk", () => {
    const enriched = enrichWithWatercourseData({
        length: 1.2,
        lengthRetained: 0.8,
        lengthEnhanced: 0.2,
    ...
    // Expected: 1.2 * 6 * 2 * 1.1 * 0.85 * 0.67 (no spatial risk)

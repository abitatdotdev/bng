...
import {
    ...
    type OffSiteHedgerowEnhancementSchema
} from "./hedgerowEnhancement";
import type { OffSiteHedgerowBaseline } from "./hedgerowBaseline";

function createBaseline(overrides: Partial<OffSiteHedgerowBaseline> = {}): OffSiteHedgerowBaseline {
    return {
        ...
        totalHedgerowUnitsSRM: 2.2, // 1 * 2 * 1 * 1.1 * 1
        totalHedgerowUnits: 2.2,
        lengthLost: 0.5,
        unitsLost: 1.1,
        ...overrides,
    } as OffSiteHedgerowBaseline;
}

export function fixture(overrides: Partial<OffSiteHedgerowEnhancementSchema> = {}): OffSiteHedgerowEnhancementSchema {
    return {
...
test("valid enhancement from Poor to Moderate", () => {
    const result = v.safeParse(offSiteHedgerowEnhancementSchema, fixture());
...
test("Non-native and ornamental hedgerow can only have Poor condition", () => {
    ...
    const resultSame = v.safeParse(offSiteHedgerowEnhancementSchema, fixture({
        baseline,
        habitatType: "Non-native and ornamental hedgerow",
        condition: "Poor"
    }));
...
test("temporal adjustment validation - cannot have both advance and delay", () => {
    // Valid: only advance
    expect(v.safeParse(offSiteHedgerowEnhancementSchema,
...
test("spatial risk multiplier - Low risk", () => {
    const baseline = createBaseline({
        spatialRiskCategory: "Compensation inside LPA boundary or NCA of impact site",
        spatialRiskMultiplier: 1,
    });
    ...
    expect(result.success).toBeTrue();
    if (result.success) {
        expect(result.output.spatialRiskMultiplier).toBe(1);
    }
});

test("spatial risk multiplier - Medium risk", () => {
    const baseline = createBaseline({
        spatialRiskCategory: "Compensation outside LPA or NCA of impact site, but in neighbouring LPA or NCA",
        spatialRiskMultiplier: 0.75,
    });
    ...
    expect(result.success).toBeTrue();
    if (result.success) {
        expect(result.output.spatialRiskMultiplier).toBe(0.75);
    }
});

test("spatial risk multiplier - High risk", () => {
    const baseline = createBaseline({
...
test("full schema validation - Native hedgerow Poor to Moderate with Low spatial risk", () => {
    const result = v.safeParse(offSiteHedgerowEnhancementSchema, fixture({
        condition: "Moderate"
    }));
    ...
    if (result.success) {
        ...
        // Final (no SRM): (0.898632125 + 1.0) * 1.1 = 2.088495
        // Final (with SRM): 2.088495 * 1 = 2.088495
        expect(result.output.hedgerowUnitsDelivered).toBeCloseTo(2.088495, 3);
        expect(result.output.hedgerowUnitsDeliveredWithSpatialRisk).toBeCloseTo(2.088495, 3);
    }
});

test("full schema validation - with Medium spatial risk", () => {
    const baseline = createBaseline({
        spatialRiskCategory: "Compensation outside LPA or NCA of impact site, but in neighbouring LPA or NCA",
        spatialRiskMultiplier: 0.75,
        totalHedgerowUnitsSRM: 1.65, // 1 * 2 * 1 * 1.1 * 0.75
    });
    ...
    if (result.success) {
        ...
        // Delta: (2.0 - 1.0) * 1 * 0.898632125 = 0.898632125
        // Final (no SRM): (0.898632125 + 1.0) * 1.1 = 2.088495
        // Final (with SRM): 2.088495 * 0.75 = 1.566371
        expect(result.output.hedgerowUnitsDelivered).toBeCloseTo(2.088495, 3);
        expect(result.output.hedgerowUnitsDeliveredWithSpatialRisk).toBeCloseTo(1.566371, 3);
    }
});

test("full schema validation - Native hedgerow to Species-rich (distinctiveness upgrade) with High spatial risk", () => {
    const baseline = createBaseline({
        habitatType: "Native hedgerow",
        condition: "Moderate",
        conditionScore: 2,
...
test("full schema validation - with advance and strategic significance", () => {
    ...
    if (result.success) {
        ...
        expect(result.output.hedgerowUnitsDeliveredWithSpatialRisk).toBeCloseTo(1.31672, 3);
    }
});
